![BenchPy](https://raw.githubusercontent.com/matteobettini/benchpy_sphinx_theme/master/benchpy_sphinx_theme/static/img/benchpy.png?raw=true)


# BenchPy
[![tests](https://github.com/khulnasoft-lab/BenchPy/actions/workflows/unit_tests.yml/badge.svg)](test)
[![codecov](https://codecov.io/github/khulnasoft-lab/BenchPy/coverage.svg?branch=main)](https://codecov.io/gh/khulnasoft-lab/BenchPy)
[![Documentation Status](https://readthedocs.org/projects/benchpy/badge/?version=latest)](https://benchpy.readthedocs.io/en/latest/?badge=latest)
[![Python](https://img.shields.io/badge/python-3.8%20%7C%203.9%20%7C%203.10-blue.svg)](https://www.python.org/downloads/)
<a href="https://pypi.org/project/benchpy"><img src="https://img.shields.io/pypi/v/benchpy" alt="pypi version"></a>
[![Downloads](https://static.pepy.tech/personalized-badge/benchpy?period=total&units=international_system&left_color=grey&right_color=blue&left_text=Downloads)](https://pepy.tech/project/benchpy)
...
[![Examples](https://img.shields.io/badge/Examples-blue.svg)](examples) [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/khulnasoft-lab/BenchPy/blob/main/notebooks/run.ipynb)
[![Static Badge](https://img.shields.io/badge/Benchmarks-Wandb-yellow)](https://wandb.ai/matteobettini/benchpy-public/reportlist)

Watch the [talk on multi-agent simulation and learning in BenchPy and TorchRL](https://www.youtube.com/watch?v=1tOIMgJf_VQ).

### What is BenchPy 🧐?

BenchPy is a Multi-Agent Reinforcement Learning (MARL) training library created to enable reproducibility
...
BenchPy uses [TorchRL](https://github.com/pytorch/rl) as its backend, which grants it high performance 
...
BenchPy **core design tenets** are:
...
Why would you BenchPy, I see you ask. 
Well, you can BenchPy to compare different algorithms, environments, models, 
...
- [BenchPy](#benchpy)
  ...
  * [Features](#features)
    ...
    + [Callbacks](#callbacks)
  * [Citing BenchPy](#citing-benchpy)


## How to use

### Notebooks
- [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/khulnasoft-lab/BenchPy/blob/main/notebooks/run.ipynb) &ensp; **Running BenchPy experiments**.
...
Experiments are launched with a [default configuration](benchpy/conf) that 
...
To achieve this, BenchPy interconnects components from [TorchRL](https://github.com/pytorch/rl), 
which provides an efficient and reliable backend.

The library has a [default configuration](benchpy/conf) for each of its components.
...
The experiment [hyperparameters](benchpy/conf/experiment/base_experiment.yaml) cover both 
...
| [IDDPG](benchpy/algorithms/iddpg.py)                                                                                                      | Off           | Yes          | No                           | Continuous            | No                  |   
| [MASAC](benchpy/algorithms/masac.py)                                                                                                      | Off           | Yes          | Yes                          | Continuous + Discrete | Yes                 |   
| [ISAC](benchpy/algorithms/isac.py)                                                                                                        | Off           | Yes          | No                           | Continuous + Discrete | Yes                 |   
...
|--------------------------------------------------------------------|-------------------------------------|---------------------------|--------------|-------------------------------|-----------------------|:----------------:|
| [VMAS](https://github.com/proroklab/VectorizedMultiAgentSimulator) | [5](benchpy/conf/task/vmas)       | Cooperative + Competitive | No           | Shared + Independent + Global | Continuous + Discrete |       Yes        |    
| [SMACv2](https://github.com/oxwhirl/smacv2)                        | [15](benchpy/conf/task/smacv2)    | Cooperative               | Yes          | Global                        | Discrete              |        No        |
| [MPE](https://github.com/openai/multiagent-particle-envs)          | [8](benchpy/conf/task/pettingzoo) | Cooperative + Competitive | Yes          | Shared + Independent          | Continuous + Discrete |        No        |
| [SISL](https://github.com/sisl/MADRL)                              | [2](benchpy/conf/task/pettingzoo) | Cooperative               | No           | Shared                        | Continuous            |        No        |

> [!NOTE]  
> BenchPy uses the [TorchRL MARL API](https://github.com/pytorch/rl/issues/1463) for grouping agents.
...
|--------------------------------|:-------------:|:-----------------------------:|:-----------------------------:|
| [MLP](benchpy/models/mlp.py) |      Yes      |              Yes              |              Yes              |
| [GNN](benchpy/models/gnn.py) |      Yes      |              No               |              No               |
...
[Wandb](https://wandb.ai/matteobettini/benchpy-public/reportlist) publicly
...
|---------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| VMAS                                  | <img src="https://raw.githubusercontent.com/matteobettini/benchpy_sphinx_theme/master/benchpy_sphinx_theme/static/img/benchmarks/vmas/environemnt_sample_efficiency_curves.png"/> | <img src="https://raw.githubusercontent.com/matteobettini/benchpy_sphinx_theme/master/benchpy_sphinx_theme/static/img/benchmarks/vmas/performance_profile_figure.png"/> | <img src="https://raw.githubusercontent.com/matteobettini/benchpy_sphinx_theme/master/benchpy_sphinx_theme/static/img/benchmarks/vmas/aggregate_scores.png"/> |

## Reporting and plotting

Reporting and plotting is compatible with [marl-eval](https://github.com/instadeepai/marl-eval). 
If `experiment.create_json=True` (this is the default in the [experiment config](benchpy/conf/experiment/base_experiment.yaml))
...
You can load and merge these files using the utils in [eval_results](benchpy/eval_results.py) to create beautiful plots of 
...
for [algorithms](benchpy/algorithms/common.py), [tasks](benchpy/environments/common.py) and [models](benchpy/models/common.py).
...
Experiment configurations are in [`benchpy/conf/config.yaml`](benchpy/conf/config.yaml).
Running custom experiments is extremely simplified by the [Hydra](https://hydra.cc/) configurations.
The default configuration for the library is contained in the [`benchpy/conf`](benchpy/conf) folder.
...
Experiment hyperparameters are loaded from [`benchpy/conf/experiment/base_experiment.yaml`](benchpy/conf/experiment/base_experiment.yaml)
into a dataclass [`ExperimentConfig`](benchpy/experiment/experiment.py) defining their domain.
...
Available algorithms and their default configs can be found at [`benchpy/conf/algorithm`](benchpy/conf/algorithm).
They are loaded into a dataclass [`AlgorithmConfig`](benchpy/algorithms/common.py), present for each algorithm, defining their domain.
This makes it so that all and only the parameters expected are loaded with the right types.
...
Available tasks and their default configs can be found at [`benchpy/conf/task`](benchpy/conf/task).
They are loaded into a dataclass [`TaskConfig`](benchpy/environments/common.py), defining their domain.
...
Available models and their configs can be found at [`benchpy/conf/model/layers`](benchpy/conf/model/layers).
They are loaded into a dataclass [`ModelConfig`](benchpy/models/common.py), defining their domain.
...
Available layer names are in the [`benchpy/conf/model/layers`](benchpy/conf/model/layers) folder.
...
BenchPy is compatible with the [TorchRL loggers](https://github.com/pytorch/rl/tree/main/torchrl/record/loggers).
A list of logger names can be provided in the [experiment config](benchpy/conf/experiment/base_experiment.yaml).
...
Experiments optionally take a list of [`Callback`](benchpy/experiment/callback.py) which have several methods
...
## License
BenchPy is licensed under the MIT License. See [LICENSE](LICENSE) for details.
